...
public class GestorSolicitudes {
	...
	public void anadirFranja(String nombreF, boolean dias[], String tipoF, int horaInicio, int horaFinal) {
		if (interfaceSolicitudes != null) {
			try {
				...
				JOptionPane.showMessageDialog(null, e.getMessage(), "�Ups! Ocurri� un error",
						JOptionPane.INFORMATION_MESSAGE);
			}
		} else {
			JOptionPane.showMessageDialog(null,
					"No se pudo a�adir una franja nueva ya que el componente de Horario no est� disponible");
		}
	}
	...
	public ArrayList<String> solicitarMaterias() {
		try {
			...
			JOptionPane.showMessageDialog(null,
					"No se pudo obtener la lista de materias porque el componente no est� disponible");
			return null;
		}
	}
